# weighted_bayesian_fitting_completed.py - 重み付き尤度関数を用いた温度依存ベイズ推定
#
...
def load_config(config_path: Optional[Union[str, pathlib.Path]] = None) -> Dict[str, Any]:
    ...
    default_config = {
        ...
        'mcmc': {
            'draws': 4000,        # 2000 → 4000 に増加
            'tune': 2000,         # 1000 → 2000 に増加
            'chains': 4,
            'target_accept': 0.90,  # デフォルト0.8から引き上げ
            'init': "adapt_diag"   # 初期化戦略を明示
        },
        'bayesian_priors': {
            'magnetic_parameters': {
            ...
            'gamma_parameters': {
                ...
                'temp_gamma_nonlinear': {'distribution': 'Normal', 'mu': 0.0, 'sigma': 0.001}
            },
            'noise_parameters': {
...
gamma_init = float(initial_values['gamma'])
...
def normalize_gamma_array(gamma_input, target_length: int = 7) -> np.ndarray:
    ...
    Args:
        ...
        target_length: 目標配列長（デフォルト7、config.ymlのshapeに応じて変更可能）
    """
...
def extract_bayesian_parameters(trace: az.InferenceData) -> Dict[str, float]:
    ...
    # 温度依存gammaパラメータも追加 (temp_gamma_nonlinear削除)
    try:
...
def create_gamma_priors(gamma_config: Dict[str, Any]) -> Dict[str, Any]:
    """gamma関連の事前分布を作成する"""
    gamma_priors = {}
    
    # log_gamma_mu_base
    cfg = gamma_config['log_gamma_mu_base']
    ...
    cfg = gamma_config['log_gamma_sigma_base']
    if cfg['distribution'] == 'HalfNormal':
        gamma_priors['log_gamma_sigma_base'] = pm.HalfNormal('log_gamma_sigma_base', 
                                                           sigma=cfg['sigma'])
    
    # log_gamma_offset_base
    cfg = gamma_config['log_gamma_offset_base']
    if cfg['distribution'] == 'Normal':
    ...
    # temp_gamma_slope
    cfg = gamma_config['temp_gamma_slope']
    if cfg['distribution'] == 'Normal':
        gamma_priors['temp_gamma_slope'] = pm.Normal('temp_gamma_slope', 
                                                   mu=cfg['mu'], 
                                                   sigma=cfg['sigma'])
    
    # temp_gamma_nonlinear - パラメータ削減のためコメントアウト
    # cfg = gamma_config['temp_gamma_nonlinear']
    # if cfg['distribution'] == 'Normal':
    #     gamma_priors['temp_gamma_nonlinear'] = pm.Normal('temp_gamma_nonlinear', 
    #                                                    mu=cfg['mu'], 
    #                                                    sigma=cfg['sigma'])
...
def run_temperature_bayesian_fit(datasets: List[Dict[str, Any]], 
    ...
    with pm.Model() as model:
        ...
        # temp_gamma_nonlinear = gamma_priors['temp_gamma_nonlinear']
        ...
        for temp in temp_list:
            ...
            # temp_correction = temp_gamma_slope * temp_diff + temp_gamma_nonlinear * temp_diff**2 # 二次項まで含む
            temp_correction = temp_gamma_slope * temp_diff # 線形項のみ
        ...
        # サンプリング設定（設定ファイルから取得）
        # 注意: カスタムOpは勾配未実装のため、PyMCの自動サンプラー選択を使用
        # (CompoundStep: Slice + Metropolis + NUTS の組み合わせ)
        try:
            # サンプリングパラメータの準備
            sample_kwargs = {
            ...
            # 収束診断の自動チェック
            print("\n--- 収束診断 ---")
            diagnostic_vars = ['a_scale', 'g_factor', 'B4', 'B6']
            convergence_issues = []
...
def plot_posterior_distributions(trace: az.InferenceData, model_type: str, results_dir: pathlib.Path):
    ...
    # プロットする主要な変数を指定 (temp_gamma_nonlinear削除)
...
def plot_temperature_dependencies(model_temperature_params: Dict[str, Dict[float, Dict[str, float]]], 
    ...
    if 'H_form' in final_traces:
        ...
        ax3.text(0.1, 0.9, 'H_formモデル 磁気パラメータ:', fontsize=14, transform=ax3.transAxes, weight='bold')
        ax3.text(0.1, 0.75, f'g因子 = {h_params["g_factor"]:.6f}', fontsize=12, transform=ax3.transAxes)
        ax3.text(0.1, 0.60, f'B4 = {h_params["B4"]:.6f} K', fontsize=12, transform=ax3.transAxes)
        ax3.text(0.1, 0.45, f'B6 = {h_params["B6"]:.6f} K', fontsize=12, transform=ax3.transAxes)
        ax3.text(0.1, 0.30, f'a_scale = {h_params["a_scale"]:.6f}', fontsize=12, transform=ax3.transAxes)
        ax3.text(0.1, 0.15, f'G0 = {h_params["G0"]:.3e}', fontsize=12, transform=ax3.transAxes)
    ax3.set_xlim(0, 1)
    ax3.set_ylim(0, 1)
    ...
    if 'B_form' in final_traces:
        ...
        ax4.text(0.1, 0.9, 'B_formモデル 磁気パラメータ:', fontsize=14, transform=ax4.transAxes, weight='bold')
        ax4.text(0.1, 0.75, f'g因子 = {b_params["g_factor"]:.6f}', fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.60, f'B4 = {b_params["B4"]:.6f} K', fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.45, f'B6 = {b_params["B6"]:.6f} K', fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.30, f'a_scale = {b_params["a_scale"]:.6f}', fontsize=12, transform=ax4.transAxes)
        ax4.text(0.1, 0.15, f'G0 = {b_params["G0"]:.3e}', fontsize=12, transform=ax4.transAxes)
    ax4.set_xlim(0, 1)
    ax4.set_ylim(0, 1)
...
def run_analysis_workflow():
    ...
    max_iterations = CONFIG['mcmc']['max_iterations']  # 更新回数の制限
