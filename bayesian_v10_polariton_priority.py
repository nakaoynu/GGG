...
SIGMA_PEAK_FREQ = 0.002  # THz (2 GHz) — v10.2: Normal尤度との組合せで厳密制約

# 【v9.0 新設定】共振器領域スペクトル形状フィッティングの σ
# v10.0: 共振器スペクトル全体の尤度は使用しない (v9のトレードオフ問題を解消)
# SIGMA_CAVITY_SPECTRUM は不要 — ピーク位置/FWHMの点制約のみ使用

# マルチピークマッチングの最大距離 [THz]
# v10.1: 0.12→0.25 に拡大 (7T/9Tでマッチング失敗していたため)
PEAK_MATCH_MAX_DISTANCE = 0.25
            ...
            if ds_idx in pred_peaks_cache:
                ...
                if len(pred_freqs) > 0:
                    ...
                    if distances[best_idx] < PEAK_MATCH_MAX_DISTANCE:
                        ...
                        # v10.1: マッチング距離超過でも最近傍ピークを使用
                        # (固定オフセットの代わりに実際の予測値を使い、
                        #  サンプラーに連続的な勾配情報を提供)
                        peak_freq_pred_list.append(pred_freqs[best_idx])
                        fwhm_pred_list.append(pred_fwhms[best_idx])
                        matched = True
...
def compute_model_evaluation(trace, model_name='Model'):
    ...
    try:
        ...
        # v10.1: 収束診断 — ESS 低下・R-hat 高値の自動警告
        convergence_warnings = []
        if 'ess_bulk' in summary.columns:
            low_ess = summary[summary['ess_bulk'] < 1000]
            if len(low_ess) > 0:
                for param_name in low_ess.index:
                    ess_val = low_ess.loc[param_name, 'ess_bulk']
                    rhat_val = low_ess.loc[param_name, 'r_hat'] if 'r_hat' in low_ess.columns else np.nan
                    msg = f"⚠️ {param_name}: ESS={ess_val:.0f} (< 1000), R-hat={rhat_val:.3f}"
                    print(f"  {msg}")
                    convergence_warnings.append({
                        'parameter': param_name,
                        'ess_bulk': float(ess_val),
                        'r_hat': float(rhat_val) if not np.isnan(rhat_val) else None
                    })
        if 'r_hat' in summary.columns:
            high_rhat = summary[summary['r_hat'] > 1.01]
            already_warned = {w['parameter'] for w in convergence_warnings}
            for param_name in high_rhat.index:
                if param_name not in already_warned:
                    rhat_val = high_rhat.loc[param_name, 'r_hat']
                    ess_val = high_rhat.loc[param_name, 'ess_bulk'] if 'ess_bulk' in high_rhat.columns else np.nan
                    msg = f"⚠️ {param_name}: R-hat={rhat_val:.3f} (> 1.01), ESS={ess_val:.0f}"
                    print(f"  {msg}")
                    convergence_warnings.append({
                        'parameter': param_name,
                        'ess_bulk': float(ess_val) if not np.isnan(ess_val) else None,
                        'r_hat': float(rhat_val)
                    })
        if convergence_warnings:
            print(f"  ⚠️ 合計 {len(convergence_warnings)} パラメータで収束に懸念あり")
        else:
            print(f"  ✅ 全パラメータで収束良好 (ESS≥1000, R-hat≤1.01)")
        result['convergence_warnings'] = convergence_warnings
...
def plot_posterior_predictive_spectra(trace, datasets, model_form='H', save_dir=None, n_samples=300):
    ...
    all_metrics = []  # v10.1: RMSE メトリクス収集用
    posterior    = trace.posterior
    ...
    fig.suptitle(f'Posterior Predictive Spectra ({model_form}-form) — v10.1 Polariton Priority',
                 fontsize=12, y=0.995)
    axes = axes.flatten()
    ...
    for idx, data in enumerate(datasets):
        ...
        dataset_metrics = {
            ...
            'rmse_polariton': float(rmse_pol) if not np.isnan(rmse_pol) else None,
            'rmse_cavity': float(rmse_cav) if not np.isnan(rmse_cav) else None,
            'peak_err_ghz': float(peak_err_ghz) if not np.isnan(peak_err_ghz) else None,
            'fwhm_obs_ghz': float(fwhm_obs * 1000) if fwhm_obs else None,
            'fwhm_pred_ghz': float(fwhm_pred * 1000) if fwhm_pred else None,
        }
    ...
    # v10.1: メトリクスサマリーを計算して返す
    if all_metrics:
        avg_rmse = np.mean([m['rmse_total'] for m in all_metrics])
        avg_rmse_pol = np.nanmean([m['rmse_polariton'] for m in all_metrics if m['rmse_polariton'] is not None])
        avg_rmse_cav = np.nanmean([m['rmse_cavity'] for m in all_metrics if m['rmse_cavity'] is not None])
        print(f"  平均 RMSE: total={avg_rmse:.4f}, polariton={avg_rmse_pol:.4f}, cavity={avg_rmse_cav:.4f}")
    return all_metrics
...
def plot_posterior_distributions(trace, model_form='H', save_dir=None):
    ...
    fig.suptitle(f'Posterior Distributions ({model_form}-form) — v10.1', fontsize=14)
    axes = axes.flatten()
...
def build_pymc_model(datasets, model_form, v6_params_H, v6_params_B):
    """
    v10.1 複合尤度モデルを構築して返す。

    尤度構造:
    - ポラリトン領域: StudentT(ν=4) で trans_pred vs trans_obs (最優先)
    - 共振器ピーク位置: Normal で peak_freq_pred vs peak_freq_obs
    - 共振器 FWHM: StudentT(ν=4) で fwhm_pred vs fwhm_obs
    ...
    with model:
        ...
        # ------------------------------------------
        # 4. B₆: Normal + clip (v10.1: σ=2mK に狭小化, 識別不能対策)
        # ------------------------------------------
        B6_raw_name = f'B6_raw_{model_form}'
        B6_raw = pm.Normal(B6_raw_name, mu=0, sigma=0.002)
        B6_scaled = pm.Deterministic('B6_scaled',
            pt.clip(B6_raw, -0.010, 0.010) * SCALING_FACTORS['B6'])
        ...
        if len(fwhm_obs_vec) > 0:
            ...
            # 8-b2. 共振器マルチピーク位置マッチング (v10.2: Normal尤度で厳密制約)
            # StudentTの重い裾 (log(1+x²) ∝ penalty) では polariton尤度に対して弱すぎる
            # Normal (x² ∝ penalty) に変更し、eps_bgを共振器ピーク側にも制約
            dist_peak = pm.Normal.dist(mu=peak_freq_pred_vec, sigma=sigma_peak_freq)
...
def main():
    ...
    print(f"\n{'='*80}")
    print("Bayesian Analysis v10.1 — Polariton Priority + Cavity Peak Constraint")
    print(f"v10.0→v10.1: ピークマッチング改善, 収束診断強化, RMSE出力, B₆事前分布狭小化")
    print(f"{'='*80}")
    ...
    metrics_H = plot_posterior_predictive_spectra(trace_H, datasets, 'H', results_dir)
    plot_posterior_distributions(trace_B, 'B', results_dir)
    metrics_B = plot_posterior_predictive_spectra(trace_B, datasets, 'B', results_dir)
